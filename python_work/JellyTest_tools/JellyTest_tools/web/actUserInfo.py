...
class actUserInfo(userInfo.getUserInfo):
	...
	def __init__(self, userid):
		...
		reqVal = request.values
		opAct = reqVal.get('activity')[2:]
	...
	def actUserInfo(self):
		...
		reqVal = request.values
		opAct = reqVal.get('activity')[2:]
		if reqVal.get('activity')[0] == 's':
			outcome = self.delete_Activity_single(opAct)
		elif reqVal.get('activity')[0] == 't':
			outcome = self.delete_Activity_team(opAct)
		elif reqVal.get('activity')[0] == 'o':
			outcome = self.delete_Activity_others(opAct)
		return outcome
